...
public class EditProductActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        tvCategory.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                new AlertDialog.Builder(EditProductActivity.this).setTitle("Choose ...")
    ...
    public void getDataAndPutItInView(){
        ...
        Log.d("ID",productId);
        ref.child(firebaseAuth.getUid()).child("Product").child(productId).orderByChild("uid").addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ModelProduct pd = snapshot.getValue(ModelProduct.class);
                edtTitle.setText(pd.getProductTitle());
                edtDescription.setText(pd.getProductDescription());
                tvCategory.setText(pd.getProductCategory());
                edtQuantity.setText(pd.getProductQuantity());
                edtPrice.setText(pd.getOriginalPrice());

                if(pd.getDiscountAvailable().equals("true")){
                    switchDiscount.setChecked(true);
                    edtDiscountPrice.setText(pd.getDiscountPrice());
                    edtDiscountNote.setText(pd.getDiscountNote());
                }else{
                ...
                try {
                       Picasso.get().load(pd.getProductIcon()).placeholder(R.drawable.ic_shipping_gray).into(imgProfile);
                }catch(Exception e){
                    e.printStackTrace();
                    imgProfile.setImageResource(R.drawable.ic_shipping_gray);
                }
            }
    ...
    private void saverFirebaseData() {
        ...
        if (uri == null) {
            ...
            databaseReference.child(firebaseAuth.getUid()).child("Product").child(productId).updateChildren(hashMap).addOnSuccessListener(new OnSuccessListener<Void>() {
                @Override
                public void onSuccess(Void aVoid) {
                    ...
                    .addOnFailureListener(new OnFailureListener() {
                        ...
                        public void onFailure(@NonNull Exception e) {
                            progressDialog.dismiss();
                            Toast.makeText(EditProductActivity.this, "Fail" + e.getMessage(), Toast.LENGTH_SHORT).show();
                        }
                    });
